...
def test_create_user(session):
    ...
    assert session.query(User).filter_by(email=user_data["email"]).first() is not None
...
def test_delete_user(session):
    ...
    user_repository = UserRepository(db_session=session)
    user = user_repository.create_user(user_data)
    created_user = session.query(User).filter_by(email=user_data["email"]).first()
    assert created_user is not None
    user_repository.delete_user(created_user.id)
    deleted_user = session.query(User).filter_by(email=user_data["email"]).first()
